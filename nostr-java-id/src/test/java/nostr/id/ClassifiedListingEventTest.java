...
class ClassifiedListingEventTest {
  public static final PublicKey senderPubkey =
      new PublicKey(Identity.generateRandomIdentity().getPublicKey().toString());
  ...
  void setup() {
    ...
    tags.add(E_TAG);
    tags.add(P_TAG);
    tags.add(BaseTag.create(TITLE_CODE, CLASSIFIED_LISTING_TITLE));
    tags.add(BaseTag.create(SUMMARY_CODE, CLASSIFIED_LISTING_SUMMARY));
    ...
    instance =
        new ClassifiedListingEvent(
            senderPubkey, Kind.CLASSIFIED_LISTING, tags, CLASSIFIED_LISTING_CONTENT);
