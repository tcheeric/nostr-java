...
public class GenericTagQueryFilter<T extends GenericTagQuery> extends AbstractFilterable<T> {
  ...
  public Predicate<GenericEvent> getPredicate() {
    return (genericEvent) ->
        Filterable.getTypeSpecificTags(GenericTag.class, genericEvent).stream()
            .filter(genericTag -> genericTag.getCode().equals(stripLeadingHashTag()))
            .anyMatch(
                genericTag ->
                    genericTag.getAttributes().stream()
                        .map(ElementAttribute::value)
                        .toList()
                        .contains(getFilterableValue()));
  }
  ...
  private String stripLeadingHashTag() {
    return getFilterKey().startsWith(HASH_PREFIX) ? getFilterKey().substring(1) : getFilterKey();
  }
