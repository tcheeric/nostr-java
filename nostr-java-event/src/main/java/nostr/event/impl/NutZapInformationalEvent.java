...
public class NutZapInformationalEvent extends ReplaceableEvent {
    ...
    protected void validateTags() {
        ...
        // At least one relay
        boolean hasValidRelayTag = this.getTags().stream()
        ...
        boolean hasValidMintTag = this.getTags().stream()
                .anyMatch(tag -> tag instanceof GenericTag && "u".equals(tag.getCode()));
        if (!hasValidMintTag) {
            throw new AssertionError("Invalid `tags`: Must include at least one valid mint tag with code 'u'.");
        }
        ...
        if (!hasValidPubKeyTag) {
            throw new AssertionError("Invalid `tags`: Must include exactly one pubkey tag with code 'pubkey'.");
        }
    }
