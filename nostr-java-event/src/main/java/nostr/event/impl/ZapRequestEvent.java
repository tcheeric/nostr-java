...
public class ZapRequestEvent extends GenericEvent {
    ...
    public ZapRequest getZapRequest() {
        GenericTag relaysTag = getTag("relays");
        GenericTag amountTag = getTag("amount");
        GenericTag lnUrlTag = getTag("lnurl");

        return new ZapRequest(
                GenericTag.convert(relaysTag, RelaysTag.class),
                Long.parseLong(amountTag.getAttributes().get(0).getValue().toString()),
                lnUrlTag.getAttributes().get(0).getValue().toString()
        );
    }
