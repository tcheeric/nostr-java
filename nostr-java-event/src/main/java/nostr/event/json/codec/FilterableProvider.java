...
public class FilterableProvider {
  protected static List<Filterable> getFilterFunction(
      @NonNull JsonNode node, @NonNull String type) {
    return switch (type) {
      case ReferencedPublicKeyFilter.FILTER_KEY ->
          getFilterable(node, ReferencedPublicKeyFilter.fxn);
  ...
  private static List<Filterable> getFilterable(
      JsonNode jsonNode, Function<JsonNode, Filterable> filterFunction) {
