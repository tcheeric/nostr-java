...
public class NIP01 extends EventNostr {
     ...
     * @param content the note's content
     */
    public NIP01 createEphemeralEvent(List<GenericTag> tags, Integer kind, String content) {
        var sender = getSender();
        GenericEvent genericEvent = new GenericEventFactory(sender, kind, (List<BaseTag>) (List<?>) tags, content).create();
        this.updateEvent(genericEvent);
        return this;
     ...
     */
    public static GenericTag createEventTag(@NonNull String relatedEventId) {
        List<String> params = List.of(relatedEventId);
        GenericTag genericTag = new GenericTagFactory(Constants.Tag.EVENT_CODE, params).create();
        return genericTag;
    }
     ...
     */
    public static GenericTag createEventTag(@NonNull String idEvent, String recommendedRelayUrl, Marker marker) {
     ...
     * recommended relay and marker
     */
    public static GenericTag createEventTag(@NonNull String idEvent, Marker marker) {
     ...
     * recommended relay and marker
     */
    public static GenericTag createEventTag(@NonNull String idEvent, Relay recommendedRelay, Marker marker) {
     ...
     * key
     */
    public static GenericTag createPubKeyTag(@NonNull PublicKey publicKey) {
        List<String> params = new ArrayList<>();
        params.add(publicKey.toString());

        GenericTag genericTag = new GenericTagFactory(Constants.Tag.PUBKEY_CODE, params).create();
    ...
    public static GenericTag createPubKeyTag(@NonNull PublicKey publicKey, String mainRelayUrl, String petName) {
        List<String> params = new ArrayList<>();
        params.add(publicKey.toString());
     ...
     */
    // TODO - Method overloading with Relay as second parameter
    public static GenericTag createPubKeyTag(@NonNull PublicKey publicKey, String mainRelayUrl) {
        List<String> params = new ArrayList<>();
        params.add(publicKey.toString());
        params.add(mainRelayUrl);
     ...
     * @return
     */
    public static GenericTag createIdentifierTag(@NonNull String id) {
        List<String> params = new ArrayList<>();
        params.add(id);

        GenericTag genericTag = new GenericTagFactory(Constants.Tag.IDENTITY_CODE, params).create();
     ...
     * @return
     */
    public static GenericTag createAddressTag(@NonNull Integer kind, @NonNull PublicKey publicKey, GenericTag idTag, Relay relay) {
        ...
        if (idTag != null) {
            if (idTag instanceof IdentifierTag) {
                param += ((IdentifierTag) idTag).getId();
            } else {
                param += idTag.getAttributes().get(0).getValue().toString();
            }
        }
        params.add(param);
        ...
        GenericTag genericTag = new GenericTagFactory(Constants.Tag.ADDRESS_CODE, params).create();

        return genericTag;
    }

    public static GenericTag createAddressTag(@NonNull Integer kind, @NonNull PublicKey publicKey, String id, Relay relay) {
        return createAddressTag(kind, publicKey, createIdentifierTag(id), relay);
    }

    public static GenericTag createAddressTag(@NonNull Integer kind, @NonNull PublicKey publicKey, String id) {
