...
public class NIP25 extends EventNostr {
     ...
     */
    public NIP25 createReactionEvent(@NonNull GenericTag eventTag, @NonNull GenericTag emojiTag) {

        // 1. Validation
        if (!emojiTag.getCode().equals(Constants.Tag.EMOJI_CODE)) {
            throw new IllegalArgumentException("The tag is not a custom emoji tag");
        }

        if (!eventTag.getCode().equals(Constants.Tag.EVENT_CODE)) {
            throw new IllegalArgumentException("The tag is not an event tag");
        }

        String shortCode = emojiTag.getAttributes().get(0).getValue().toString();
    ...
    public static GenericTag createKindTag(@NonNull Integer kind) {
        return new GenericTagFactory(Constants.Tag.KIND_CODE, kind.toString()).create();
    }

    public static GenericTag createCustomEmojiTag(@NonNull String shortcode, @NonNull URL url) {
        return new GenericTagFactory(Constants.Tag.EMOJI_CODE, shortcode, url.toString()).create();
    }

    @SneakyThrows
    public static GenericTag createCustomEmojiTag(@NonNull String shortcode, @NonNull String url) {
