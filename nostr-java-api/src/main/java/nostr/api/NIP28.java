...
public class NIP28 extends EventNostr {
   ...
   * @param messageReplyTo the reply tag. If present, it must be a reply to a message, else it is a
   *     root message
   * @param recommendedRelayRoot in the scenario of a root message, the recommended relay for the
   *     root message
   * @param recommendedRelayReply in the scenario of a reply message, the recommended relay for the
   *     reply message
  ...
  public NIP28 createChannelMessageEvent(
    ...
    // 3. Add the tags
    genericEvent.addTag(
        NIP01.createEventTag(channelCreateEvent.getId(), recommendedRelayRoot, Marker.ROOT));
    if (messageReplyTo != null) {
      genericEvent.addTag(
          NIP01.createEventTag(messageReplyTo.getId(), recommendedRelayReply, Marker.REPLY));
   ...
   * @param profile the channel metadata
   */
  public NIP28 updateChannelMetadataEvent(
      @NonNull GenericEvent channelCreateEvent, @NonNull ChannelProfile profile, Relay relay) {
  ...
  public NIP28 updateChannelMetadataEvent(
      @NonNull GenericEvent channelCreateEvent,
      @NonNull ChannelProfile profile,
