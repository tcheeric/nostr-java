...
public class NIP65 extends EventNostr {
    ...
    public NIP65 createRelayListMetadataEvent(@NonNull List<Relay> relayList) {
        List<BaseTag> relayUrlTags = relayList.stream()
                .map(relay -> createRelayUrlTag(relay))
                .toList();
        GenericEvent genericEvent = new GenericEventFactory(getSender(), Constants.Kind.RELAY_LIST_METADATA_EVENT, relayUrlTags, "").create();
    ...
    public NIP65 createRelayListMetadataEvent(@NonNull List<Relay> relayList, @NonNull Marker permission) {
        List<BaseTag> relayUrlTags = relayList.stream()
                .map(relay -> createRelayUrlTag(relay, permission))
                .toList();
        GenericEvent genericEvent = new GenericEventFactory(getSender(), Constants.Kind.RELAY_LIST_METADATA_EVENT, relayUrlTags, "").create();
        this.updateEvent(genericEvent);
        return this;
