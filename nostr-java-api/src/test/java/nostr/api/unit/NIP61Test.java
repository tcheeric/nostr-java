...
public class NIP61Test {
  ...
  public void createNutzapInformationalEvent() {
    ...
    List<Relay> relays =
        Arrays.asList(new Relay("wss://relay1.example.com"), new Relay("wss://relay2.example.com"));
    List<CashuMint> mints =
        Arrays.asList(
            new CashuMint("https://mint1.example.com"), new CashuMint("https://mint2.example.com"));
    ...
    Assertions.assertEquals(
        "wss://relay1.example.com", relayTags.get(0).getAttributes().get(0).value());
    Assertions.assertEquals(
        "wss://relay2.example.com", relayTags.get(1).getAttributes().get(0).value());
    ...
    Assertions.assertEquals(
        "https://mint1.example.com", mintTags.get(0).getAttributes().get(0).value());
    Assertions.assertEquals(
  ...
  public void createNutzapEvent() {
    ...
    // Verify url tag
    List<BaseTag> urlTags = tags.stream().filter(tag -> tag.getCode().equals("u")).toList();
    ...
    // Verify amount tag
    List<BaseTag> amountTags = tags.stream().filter(tag -> tag.getCode().equals("amount")).toList();
    ...
    // Verify unit tag
    List<BaseTag> unitTags = tags.stream().filter(tag -> tag.getCode().equals("unit")).toList();
    ...
    // Verify pubkey tag
    List<BaseTag> pubkeyTags = tags.stream().filter(tag -> tag.getCode().equals("p")).toList();
    assertInstanceOf(PubKeyTag.class, pubkeyTags.get(0));
    Assertions.assertEquals(1, pubkeyTags.size());
    Assertions.assertEquals(
        recipientId.getPublicKey().toString(),
        ((PubKeyTag) pubkeyTags.get(0)).getPublicKey().toString());
