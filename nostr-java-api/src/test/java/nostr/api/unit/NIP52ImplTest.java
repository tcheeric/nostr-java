...
class NIP52ImplTest {
  ...
  public static final String CALENDAR_TIME_BASED_EVENT_SUMMARY =
      "Calendar Time-Based Event listing summary";
  ...
  public static final PubKeyTag P_2_TAG =
      new PubKeyTag(new PublicKey(PTAG_2_HEX), null, "COUNTERPARTY");
  ...
  void testNIP52CreateTimeBasedCalendarCalendarEventWithAllOptionalParameters() {
    ...
    GenericEvent calendarTimeBasedEvent =
        nip52
            .createCalendarTimeBasedEvent(tags, TIME_BASED_EVENT_CONTENT, timeBasedCalendarContent)
            .getEvent();
    ...
    assertTrue(
        calendarTimeBasedEvent.getTags().contains(containsGeneric("title", TIME_BASED_TITLE)));
    assertTrue(
        calendarTimeBasedEvent.getTags().contains(containsGeneric("start", START.toString())));
    ...
    assertTrue(
        calendarTimeBasedEvent
            .getTags()
            .contains(containsGeneric("summary", CALENDAR_TIME_BASED_EVENT_SUMMARY)));
    assertTrue(calendarTimeBasedEvent.getTags().contains(P_1_TAG));
    assertTrue(calendarTimeBasedEvent.getTags().contains(P_2_TAG));
    assertTrue(
        calendarTimeBasedEvent
            .getTags()
            .contains(containsGeneric("location", CALENDAR_TIME_BASED_EVENT_LOCATION)));
    ...
    CalendarContent<BaseTag> calendarContent =
        new CalendarContent<>(identifierTag, TIME_BASED_TITLE, START);
