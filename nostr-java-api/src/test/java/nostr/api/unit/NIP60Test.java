...
public class NIP60Test {
  ...
  public void createWalletEvent() throws JsonProcessingException {
    ...
    // Assert relay tags
    List<BaseTag> relayTags = tags.stream().filter(tag -> tag.getCode().equals("relay")).toList();

    Assertions.assertEquals(2, relayTags.size());

    // Assert mint tags
    List<BaseTag> mintTags = tags.stream().filter(tag -> tag.getCode().equals("mint")).toList();
