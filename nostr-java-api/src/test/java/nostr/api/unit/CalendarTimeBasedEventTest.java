...
class CalendarTimeBasedEventTest {
  ...
  public static final String CALENDAR_TIME_BASED_EVENT_CONTENT =
      "calendar Time-Based Event content";
  public static final IdentifierTag identifierTag =
  ...
  public static final String PTAG_1_HEX =
  ...
  public static final String PTAG_2_HEX =
      "494001ac0c8af2a10f60f23538e5b35d3cdacb8e1cc956fe7a16dfa5cbfc4347";
  public static final PubKeyTag P_2_TAG =
      new PubKeyTag(new PublicKey(PTAG_2_HEX), str, "COUNTERPARTY");
  ...
  public static final GeohashTag G_TAG =
      new GeohashTag("Calendar Time-Based Event Test Geohash Tag");
  public static final HashtagTag T_TAG =
      new HashtagTag("Calendar Time-Based Event Test Hashtag Tag");

  public static final String CALENDAR_TIME_BASED_EVENT_SUMMARY =
      "Calendar Time-Based Event summary";
  ...
  public static final String CALENDAR_TIME_BASED_EVENT_LOCATION =
      "Calendar Time-Based Event location";
  ...
  void setup() throws URISyntaxException {
    ...
    CalendarContent<BaseTag> calendarContent =
        new CalendarContent<>(identifierTag, CALENDAR_TIME_BASED_EVENT_TITLE, START);
    ...
    instance =
        new NIP52(identity)
            .createCalendarTimeBasedEvent(tags, CALENDAR_TIME_BASED_EVENT_CONTENT, calendarContent)
            .getEvent();
    ...
    expectedEncodedJson =
        "{"
            ...
            + "[\"p\",\"494001ac0c8af2a10f60f23538e5b35d3cdacb8e1cc956fe7a16dfa5cbfc4347\",\"http://some.url\",\"COUNTERPARTY\"],[\"location\",\"Calendar"
  ...
  void testCalendarTimeBasedEventEncoding() throws JsonProcessingException {
    ...
    assertEquals(
        findTagArray.apply(instanceJson.get("tags"), "d").get(1).asText(),
        findTagArray.apply(expectedJson.get("tags"), "d").get(1).asText());
    assertEquals(
        findTagArray.apply(instanceJson.get("tags"), "title").get(1).asText(),
        findTagArray.apply(expectedJson.get("tags"), "title").get(1).asText());
    assertEquals(
        findTagArray.apply(instanceJson.get("tags"), "start").get(1).asText(),
        findTagArray.apply(expectedJson.get("tags"), "start").get(1).asText());
  }
  ...
  void testCalendarTimeBasedEventDecoding() throws JsonProcessingException {
    ...
    assertEquals(
        findTagArray.apply(decodedTags, "d").get(1).asText(),
        findTagArray.apply(instanceTags, "d").get(1).asText());
    assertEquals(
        findTagArray.apply(decodedTags, "title").get(1).asText(),
        findTagArray.apply(instanceTags, "title").get(1).asText());
    assertEquals(
        findTagArray.apply(decodedTags, "start").get(1).asText(),
        findTagArray.apply(instanceTags, "start").get(1).asText());
  }
}
